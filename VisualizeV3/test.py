...
if __name__  == '__main__':
    ...
    s1a.add_measurement(mtype='mass', value=60.0, unit='mg')
    h1b2 = s1b.add_measurement(mtype='hys', mfile=join(folder, 'FeNi_FeNi20-Jd120-G03_HYS_VSM#50,8[mg]_[]_[]##STD015.001'), machine='vsm')
    s1a.add_measurement(mtype='mass', value=50.8, unit='mg')
    # adding series parapmeters
    h1a1.add_series(stype='mtime', sval=1)
    h1a2.add_series(stype='mtime', sval=30)
    h1b1.add_series(stype='mtime', sval=60)
    h1b2.add_series(stype='mtime', sval=120)
    ...
    s1 = fig.add_visual(visual='resultvsseries', result='ms', series='mtime', name='coe', plt_input=sg1)
    print s1.xlabel
